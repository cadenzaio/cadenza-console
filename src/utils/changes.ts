...
import Cadenza, { Task } from "@cadenza.io/core";

const nodeToTask: { [key: string]: Task } = {};
...
function applyChanges(changes: any[], elements: any[]): any[] {
    ...
    if (addItemChanges.length) {
        addItemChanges.forEach((change) => {
            ...
            nodeToTask[change.item.id] = Cadenza.createTask(
                change.item.data.label, // @ts-ignore
                new Function("ctx", "emit", "progressCallback", `return ${change.item.data.code}`),
                change.item.data.description,
            );
        });
    }

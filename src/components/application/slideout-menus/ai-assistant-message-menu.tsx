        ...
        <div ref={ menuRef }>
            <SlideoutMenu.Trigger isOpen={ isOpen } onOpenChange={ setIsOpen }>
                <SlideoutMenu isDismissable className="overflow-auto">
                    <SlideoutMenu.Header
                        className="sticky top-0 right-0 left-0 bg-primary/70 px-4 pt-6 pb-8 backdrop-blur-sm md:px-6">
                        <Logo />
                    </SlideoutMenu.Header>
                    <SlideoutMenu.Content
                        className="size-auto w-full gap-0 overflow-visible overscroll-none p-0 md:px-0">
                        <ol aria-label="Conversation" className="flex flex-col px-4 pb-8 md:px-6">
                            { messages.map( ( msg ) => {
                                            ...
                                            { msg.meetings.map( ( meeting ) => (
                                                <div key={ meeting.id } className="flex gap-2">
                                                    ...
                                                    <p className="mt-0.5 text-sm text-tertiary">
                                                        <span
                                                            className="font-semibold text-primary underline underline-offset-2">{ meeting.title }</span>{ " " }
                                                        { meeting.description }.
                                                    </p>
                                                </div>
                                            ) ) }
                                ...
                                if ( msg.actions ) {
                                    return (
                                        <li key={ msg.id } data-self={ msg.user?.me }
                                            className={ cx( "peer flex flex-col gap-3", marginTop ) }>
                                ...
                                return <MessageItem key={ msg.id } data-self={ msg.user?.me } msg={ msg }
                                                    className={ cx( "peer", marginTop ) } showUserLabel={ false } />;
                            } ) }
                        </ol>
